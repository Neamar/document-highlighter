...
module.exports = function highlightHtml(html, query, options) {
  ...
  while(true) {
    ...
    if(isClosingElement) {
      ...
      if(isBlock) {
        // At the end of a block element, add a space separator.
      ...
      // Skip any attributes
      var attributesPosition = markupElement.indexOf(' ');
      if(attributesPosition !== -1) {
        markupElement = markupElement.substr(0, attributesPosition);
      }
    ...
    // c: is a closing tag
    // shift: amount by which HTML is displaced with respect to text
    // isBlock: whether or not this element is block-level
    mappings.push({
      t: text.length,
      h: endIndex,
  ...
  var getMapping = function(textStartIndex, textEndIndex) {
    ...
    for(var i = 0; i < mappings.length; i += 1) {
      ...
      if(mapping.t < textStartIndex) {
        ...
        if(!mapping.c && mappings[i + 1] && mappings[i + 1].c && textEndIndex >= mappings[i + 1].t && !mapping.isBlock) {
          ...
          var subMappingEnd =  mapping.h - (mapping.l + mapping.shift) + 1;
      ...
      htmlStartDelta -= mapping.shift;
      htmlEndDelta += mapping.l - mapping.shift;
    }
